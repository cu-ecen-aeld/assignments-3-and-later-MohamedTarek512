...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
   ...
   if ! grep -q 'KBUILD_CFLAGS += $(call cc-option,-fcf-protection=none)' Makefile; then
        sed -i '682i KBUILD_CFLAGS += $(call cc-option,-fcf-protection=none)' Makefile
        echo "line added"
    fi
    make clean
    make defconfig
...
mkdir -p ${OUTDIR}/rootfs/{bin,dev,etc,home,lib,proc,sbin,sys,usr/{bin,sbin},var}
cd "$OUTDIR"
if [ ! -d "${OUTDIR}/busybox" ]
then
